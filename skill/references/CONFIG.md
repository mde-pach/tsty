...
|-------|------|----------|---------|-------------|
| `baseUrl` | string | ✅ Yes | - | Base URL for all relative URLs in tests |
...
**⚠️ Warning**: Only use these for testing, not production!
...
**⚠️ CRITICAL: Use `headless: true` for all autonomous/agentic testing**

**Headless** (`headless: true` - default & recommended):
- ✅ **REQUIRED for autonomous/agentic testing**
...
- ⚠️ **DO NOT USE for autonomous testing**
...
❌ **DON'T**: Hardcode production URLs in config
```json
{
...
✅ **DO**: Use localhost by default, override in CI/CD
```json
{
  "baseUrl": "http://localhost:3000"
}
...
✅ **DO**: Use test accounts with limited permissions
```json
{
  "credentials": {
...
❌ **DON'T**: Use extremely long timeouts
```json
{
  "playwright": {
...
✅ **DO**: Use appropriate timeouts per scenario
```json
{
  "playwright": {
